  ...
  <div>
    ...
    />
        ...
        <el-select v-model="ruleForm.first_cateid" placeholder="请选择父级菜单">
          <el-option v-for="(item,index) in options" :key="item.id" :label="item.catename" :value="item.id" @click="GetOneIdx(item.id)" />
        </el-select>
      </el-form-item>
     ...
     <el-form-item label="二级目录">
        <el-select v-model="ruleForm.second_cateid" placeholder="请选择父级菜单">
          <el-option v-for="(item,index) in options[OneIdx].children" :key="item.id" :label="item.catename" :value="item.id"  />
        </el-select>
      </el-form-item>

     <el-form-item label="选择商品">
        <el-select v-model="ruleForm.goodsid" placeholder="请选择父级菜单">
         <el-option label="小米10" :value= '0' >
          </el-option>
...
import {getseckillone,editseckill} from "@/api/商城管理/秒杀活动"
let store = useStore();
let ruleFormRef = ref("");
...
async function getCateListOne(){
    let res = await getseckillone(props.RowId);
    console.log("1111111",res); 
    value1.value[0] = new Date(Number(res.list.begintime));
    value1.value[1]= new Date(Number(res.list.endtime));
    ruleForm.first_cateid = res.list.first_cateid;
    ruleForm.goodsid = res.list.goodsid
    ruleForm.status = res.list.status
    ruleForm.id = props.RowId
    ruleForm.second_cateid = res.list.second_cateid
...
function submitForm() {
    EditSeckill();
    store.commit("updataIsEditSeckillClose",false);
    location.reload();
}
</script>
